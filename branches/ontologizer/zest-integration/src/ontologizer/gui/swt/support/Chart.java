		...
		addPaintListener(new PaintListener(){
			public void paintControl(PaintEvent e)
			{
				...
				while (iter.hasNext())
				{
					...
					int lastX = (int)(series.x[0] * areaWidth / maxX);
					int lastY = (int)(series.y[0] * areaHeight / maxY);
					...
					for (int i = 1; i<series.x.length;i++)
					{
						...
						{
							e.gc.drawLine(
								areaLeft + (int)lastX, areaTop + areaHeight - (int)lastY - 1,
								areaLeft + (int)x, areaTop + areaHeight - (int)y_accu/y_num - 1);
